...
const ConnectionStatus: React.FC<ConnectionStatusProps> = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    if (!isConnected || connectionError) {
      ...
      // Show troubleshooting tips after 5 seconds of connection issues
      ...
      return () => clearTimeout(troubleshootingTimeout);
    } else if (isConnected && visible && !connectionError) {
  ...
  }, [isConnected, connectionError, visible]);
  ...
  if (connectionError) {
    ...
    statusMessage = `Reconnecting (${reconnectAttempts}/10)...`;
    statusIcon = (
  ...
  return (
    ...
    >
      <div 
        className={`flex flex-col rounded-lg shadow-lg overflow-hidden ${statusBg} ${statusColor} border ${statusBorder} backdrop-blur-sm animate-pop-in ring-1 ${statusRing}`}
      >
          ...
          <div>
            ...
            {reconnectAttempts > 0 && (
              <div className="mt-1">
                ...
                <p className="text-xs mt-1 opacity-80">Attempt {reconnectAttempts} of 10</p>
              </div>
            )}
            {!isConnected && !reconnectAttempts && (
              <button 
                onClick={() => window.location.reload()}
                className="text-xs mt-1 underline hover:no-underline focus:outline-none"
              >
                Refresh page if this takes too long
              </button>
            )}
          </div>
        </div>
        ...
        {showTroubleshooting && (connectionError || reconnectAttempts > 2) && (
          <div className={`p-3 text-sm border-t ${statusBorder}`}>
            <p className="font-medium mb-2">Troubleshooting tips:</p>
            <ul className="list-disc list-inside space-y-1">
              <li>Make sure the game server is running</li>
              <li>Check your internet connection</li>
              <li>Try using a different browser</li>
              <li>If you're in the game, others can still see you</li>
              <li>
                <button 
                  onClick={() => window.location.reload()}
